...
export function registerWorkspaceRoutes(app, clerkClient) {
    ...
    app.post('/api/workspaces/join', requireAuthMiddleware, async (req, res) => {
        try {
            ...
            const joined = await joinWorkspaceById(workspaceId, req.userId);
            ...
            return res.json({ ok: true, workspace: await getWorkspace(workspaceId), message: `Joined "${ws.name}".` });
        } catch (err) {
    ...
    app.post('/api/workspaces', requireAuthMiddleware, async (req, res) => {
        try {
    ...
    app.get('/api/workspaces', requireAuthMiddleware, async (req, res) => {
        try {
    ...
    app.get('/api/workspaces/:id', requireAuthMiddleware, async (req, res) => {
        try {
            ...
            if (!(await hasWorkspaceAccess(req.userId, id))) {
                return res.status(403).json({ error: 'Access denied.' });
            }

            const members = await getWorkspaceMembers(id);
            const projects = await getProjectsForWorkspace(id);
            const activities = await getWorkspaceActivity(id, 30);
            const myMember = members.find(m => m.id === req.userId);
            const currentUserRole = ws.owner_id === req.userId ? 'admin' : (myMember?.role || null);
            const rev = await getWorkspaceRevision(id);
    ...
    app.get('/api/workspaces/:id/revision', requireAuthMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id, 10);
            if (isNaN(id)) return res.status(400).json({ error: 'Invalid workspace ID.' });
    ...
    app.patch('/api/workspaces/:id', requireAuthMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id, 10);
            if (isNaN(id)) return res.status(400).json({ error: 'Invalid workspace ID.' });

            if (!(await canManageMembers(req.userId, id))) {
            ...
            return res.json({ workspace: await getWorkspace(id) });
        } catch (err) {
    ...
    app.post('/api/workspaces/:id/invite', requireAuthMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id, 10);
            if (isNaN(id)) return res.status(400).json({ error: 'Invalid workspace ID.' });
            ...
            if (userId != null) {
                ...
                if (!isNaN(uid) && (await getUserById(uid))) inviteeUserId = uid;
            }
            if (!inviteeUserId && username) {
                const u = await getUserByUsername(sanitizeStr(username, 50));
            ...
            const invUser = inviteeUserId ? await getUserById(inviteeUserId) : null;
            if (inviteeUserId) {
    ...
    app.patch('/api/workspaces/:id/members/:userId/role', requireAuthMiddleware, async (req, res) => {
        try {
            const workspaceId = parseInt(req.params.id, 10);
            const targetUserId = parseInt(req.params.userId, 10);
            if (isNaN(workspaceId) || isNaN(targetUserId)) return res.status(400).json({ error: 'Invalid ID.' });
            if (!(await canManageMembers(req.userId, workspaceId))) return res.status(403).json({ error: 'Only admins can change roles.' });
            const { role } = req.body || {};
            const newRole = role === 'admin' || role === 'member' ? role : null;
            if (!newRole) return res.status(400).json({ error: 'Role must be admin or member.' });
            const ok = await updateMemberRole(workspaceId, targetUserId, newRole);
            if (!ok) return res.status(400).json({ error: 'Could not update role.' });
            await logActivity({ userId: req.userId, workspaceId, action: 'updated_member_role', entityType: 'member', entityId: String(targetUserId), metadata: { role: newRole } });
            return res.json({ ok: true, members: await getWorkspaceMembers(workspaceId) });
        } catch (err) {
    ...
    app.get('/api/workspaces/:id/daily-tasks', requireAuthMiddleware, async (req, res) => {
        try {
            const workspaceId = parseInt(req.params.id, 10);
            if (isNaN(workspaceId)) return res.status(400).json({ error: 'Invalid workspace ID.' });
            if (!(await hasWorkspaceAccess(req.userId, workspaceId))) return res.status(403).json({ error: 'Access denied.' });
            const userId = req.query.userId ? parseInt(req.query.userId, 10) : null;
            const taskDate = sanitizeStr(req.query.date, 20) || null;
            const tasks = await getMemberDailyTasks(workspaceId, isNaN(userId) ? null : userId, taskDate);
    ...
    app.post('/api/workspaces/:id/daily-tasks', requireAuthMiddleware, async (req, res) => {
        try {
            const workspaceId = parseInt(req.params.id, 10);
            if (isNaN(workspaceId)) return res.status(400).json({ error: 'Invalid workspace ID.' });
            if (!(await hasWorkspaceAccess(req.userId, workspaceId))) return res.status(403).json({ error: 'Access denied.' });
            ...
            if (userId !== req.userId && !(await canManageMembers(req.userId, workspaceId))) return res.status(403).json({ error: 'Only admins can assign tasks to others.' });
            const member = (await getWorkspaceMembers(workspaceId)).find(m => m.id === userId);
            if (!member) return res.status(400).json({ error: 'User is not a workspace member.' });
            const taskId = await createMemberDailyTask({ workspaceId, userId, taskText: text, taskDate: date, assignedBy: req.userId });
            const task = await getMemberDailyTaskById(taskId);
    ...
    app.patch('/api/workspaces/:id/daily-tasks/:taskId', requireAuthMiddleware, async (req, res) => {
        try {
            const workspaceId = parseInt(req.params.id, 10);
            ...
            const isAdmin = await canManageMembers(req.userId, workspaceId);
            ...
            await updateMemberDailyTask(taskId, req.userId, updates);
            const updated = await getMemberDailyTaskById(taskId);
    ...
    app.delete('/api/workspaces/:id/daily-tasks/:taskId', requireAuthMiddleware, async (req, res) => {
        try {
            const workspaceId = parseInt(req.params.id, 10);
            const taskId = parseInt(req.params.taskId, 10);
            if (isNaN(workspaceId) || isNaN(taskId)) return res.status(400).json({ error: 'Invalid ID.' });
            if (!(await canManageMembers(req.userId, workspaceId))) return res.status(403).json({ error: 'Only admins can delete tasks.' });
            const task = await getMemberDailyTaskById(taskId);
            if (!task) return res.status(404).json({ error: 'Task not found.' });
            await deleteMemberDailyTask(taskId, req.userId);
    ...
    app.post('/api/invitations/:id/accept', requireAuthMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id, 10);
            if (isNaN(id)) return res.status(400).json({ error: 'Invalid invitation ID.' });

            const ok = await acceptInvitation(req.userId, id);
    ...
    app.post('/api/invitations/:id/reject', requireAuthMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id, 10);
            if (isNaN(id)) return res.status(400).json({ error: 'Invalid invitation ID.' });

            await rejectInvitation(req.userId, id);
            return res.json({ ok: true });
        } catch (err) {
    ...
    app.get('/api/invitations', requireAuthMiddleware, async (req, res) => {
        try {
            const invitations = await getInvitationsForUser(req.userId);
    ...
    app.post('/api/workspaces/:id/projects', requireAuthMiddleware, async (req, res) => {
        try {
            const workspaceId = parseInt(req.params.id, 10);
            if (isNaN(workspaceId)) return res.status(400).json({ error: 'Invalid workspace ID.' });

            if (!(await hasWorkspaceAccess(req.userId, workspaceId))) {
            ...
            await logExecution({ userId: req.userId, projectId, workspaceId, action: 'create_project', score: 5 });

            const project = await getProjectWithAssignee(projectId);
            if (!project) {
    ...
    app.patch('/api/projects/:id', requireAuthMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id, 10);
            ...
            if (!(await hasWorkspaceAccess(req.userId, project.workspace_id))) {
                return res.status(403).json({ error: 'Access denied.' });
            }
            ...
            if (assignedTo !== undefined) {
                ...
                if (assignId === null || (await getWorkspaceMembers(project.workspace_id)).some(m => m.id === assignId)) {
            ...
            if (status) {
                ...
                await logExecution({ userId: req.userId, projectId: id, workspaceId: project.workspace_id, action: 'status_update', score: scoreMap[status] || 0 });
            }

            return res.json({ project: await getProjectWithAssignee(id) });
        } catch (err) {
    ...
    app.get('/api/projects/:id/canvas', requireAuthMiddleware, async (req, res) => {
        try {
            ...
            if (wid == null || !(await hasWorkspaceAccess(req.userId, wid))) {
            ...
            const row = await getProjectCanvas(projectId);
            if (!row) {
    ...
    app.post('/api/projects/:id/canvas', requireAuthMiddleware, async (req, res) => {
        try {
            const projectId = parseInt(req.params.id, 10);
            if (isNaN(projectId)) return res.status(400).json({ error: 'Invalid project ID.' });
            const project = await getProject(projectId);
            ...
            await saveProjectCanvas(projectId, dataStr);
            return res.json({ ok: true });
        } catch (err) {
    ...
    app.get('/api/profile/:username', async (req, res) => {
        try {
            ...
            let user = await getUserByUsername(param);
            if (!user && param.includes('@')) user = await getUserByEmail(param);
            if (!user) return res.status(404).json({ error: 'User not found.' });

            const profile = await getProfile(user.id);
            const streak = await getExecutionStreak(user.id);
            const progress = await getProgressScore(user.id, null, 30);
            const activities = await getProfileActivity(user.id, 20);
            const projects = await getProfileProjects(user.id, 20);
            ...
            if (authHeader?.startsWith('Bearer ')) {
                ...
                if (sub && clerk) {
                    try {
                        ...
                        const viewerId = await getOrCreateUser(clerkUser);
                        canChat = viewerId !== user.id && (await doUsersShareWorkspace(viewerId, user.id));
                    } catch { /* ignore */ }
                }
            }
    ...
    app.get('/api/me', requireAuthMiddleware, async (req, res) => {
        try {
            const user = await getUserById(req.userId);
            const profile = await getProfile(req.userId);
            const streak = await getExecutionStreak(req.userId);
            const progress = await getProgressScore(req.userId, null, 30);
            const invitations = await getInvitationsForUser(req.userId);
            const activities = await getProfileActivity(req.userId, 20);
            const projects = await getProfileProjects(req.userId, 20);
    ...
    app.patch('/api/me/profile', requireAuthMiddleware, async (req, res) => {
        try {
            const { bio } = req.body || {};
            await updateProfile(req.userId, { bio: sanitizeStr(bio, 500) });
            return res.json({ ok: true, profile: await getProfile(req.userId) });
        } catch (err) {
    ...
    app.post('/api/execution', requireAuthMiddleware, async (req, res) => {
        try {
            ...
            const streak = await getExecutionStreak(req.userId, projectId ? parseInt(projectId, 10) : null);
    ...
    app.get('/api/notifications', requireAuthMiddleware, async (req, res) => {
        try {
            const unreadOnly = req.query.unread === 'true';
            const notifications = await getNotifications(req.userId, unreadOnly);
    ...
    app.post('/api/notifications/:id/read', requireAuthMiddleware, async (req, res) => {
        try {
            const id = parseInt(req.params.id, 10);
