...
export const FolderNode = memo(({ data, selected }: any) => {
    return (
        <div
            className={`
                min-w-[160px] relative transition-all group cursor-pointer
        ...
        >
            ...
            `}>
                ...
                `}>
                    <Folder size={24} fill={selected ? 'currentColor' : 'none'} className={selected ? 'opacity-30' : ''} />
                </div>
                <div className="text-center">
                    <span className="font-bold text-sm block tracking-wide">{data.label}</span>
                    <span className="text-[10px] text-white/40 uppercase tracking-widest font-black mt-1 block">Workspace</span>
                </div>

                {data.onOpenFolder && (
                    <button
                        onClick={(e) => { e.stopPropagation(); data.onOpenFolder(); }}
                        className={`
                            px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-xs font-bold uppercase tracking-widest mt-2
                            transition-all hover:bg-white hover:text-black hover:border-white
